...
Force = Ptest * (pi*B^2)./(2*pi*N + w).^2 .* (1 - 3.*((L+s)./B).^2);
Moment = Ptest * -(2*pi*B^3)./(2*pi*N + w).^3 .* (((L+s)./B) - ((L+s)./B).^3);
% gama = acos((L+s)./B);
% r = -B./(2*pi*N + w) * (1 - ((L+s)./B).^2).^(1/2);
% Force = Ptest * pi*r.^2.*(1 - 2.*cot(gama).^2);
% Moment = - Ptest * 2*pi*r.^3 .* cot(gama);
