function vf_visualization( csv_linear, csv_constitutive, csv_data, res, scale, xrange, yrange )
%vf_visualization
...
realData = csvread(csv_data);

% Make sure both sets of data are for same FREE and at same pressures
if linModel(1,:) ~= constModel(1,:) | constModel(1,:) ~= realData(1,:) | linModel(1,:) ~= realData(1,:)
...
[x3, y3, u3, v3] = deal( vf3_3d(:,1,:), vf3_3d(:,2,:), vf3_3d(:,3,:), vf3_3d(:,4,:) );
[p1, vol1] = deal( vf1_3d(:,5,:), vf1_3d(:,6,:) );
[p2, vol2] = deal( vf2_3d(:,5,:), vf2_3d(:,6,:) );
% [p3, vol3] = deal( vf3_3d(:,5,:), vf3_3d(:,6,:) );    % no volume info in Audrey's spreadsheet
...
for k = 1:length(P)
    ...
    F3(:,:,k) = griddata(s3(:,:,k), w3(:,:,k), f3(:,:,k), S,W);
    M3(:,:,k) = griddata(s3(:,:,k), w3(:,:,k), m3(:,:,k), S,W);
end
...
for i = 1:length(P)
    ...
    quiv3(i) = quiver(S,W, F3(:,:,i), M3(:,:,i), 'Color', 'black', 'Linewidth', 1, 'MaxHeadSize', 5e-2, 'AutoScale', 'off', 'ShowArrowHead', 'off', 'Marker', '.');
end
hold off
legend([quiv1(1), quiv2(1), quiv3(1)], {'Linear Model', 'Constitutive Model', 'Measured Data'}, 'Fontsize', 24, 'Location', 'eastoutside');
...
for i = 1:length(P)
    ...
    U3 = reshape(quiv3(i).UData, [numel(quiv3(i).UData),1]);
    V3 = reshape(quiv3(i).VData, [numel(quiv3(i).VData),1]);
    X = reshape(quiv1(i).XData, [numel(quiv1(i).XData),1]);
    Y = reshape(quiv1(i).YData, [numel(quiv1(i).YData),1]);
    
    fade = 0.75/length(P);
    arrow3( [X,Y], [X+U1,Y+V1] , '^r', 0.5, 0.5, [], 1, i*fade);
    arrow3( [X,Y], [X+U2,Y+V2] , '^b', 0.5, 0.5, [], 1, i*fade);
    arrow3( [X,Y], [X+U3,Y+V3] , '^d', 0.5, 0.5, [], 1, i*fade);
end
...
for i = 1:length(P)
    ...
    annotation('rectangle', [xBox+2*wBox, yBox + (i-1)*0.025, 0.025, 0.025], 'FaceColor', [0.2*(i-1) 0.2*(i-1) 0.2*(i-1)]);
end

% Label the color boxes with corresponding pressures
for i = 1:length(P)
    dim = [xBox+3*wBox, yBox + (i-1)*hBox, 0.1, hBox];
    str = {num2str(P(i)) 'kPa'};
    str = strjoin(str);
