...
params.R_l = 1.5e-2;    % relaxed FREE radius (m)
...
params.Ptest_l = 69e3; % test pressure (Pa). Not needed for most functions.

% Parameters for the left FREE, designated by postscript "_r"
params.Gama_r = 0.75;  % relaxed fiber angle (rad)
params.R_r = 1.5e-2;    % relaxed FREE radius (m)
...
params.Ptest_r = 69e3; % test pressure (Pa). Not needed for most functions.
...
% Left FREE
params.B_l = params.L_l/cos(params.Gama_l);   % fiber length
params.N_l = -params.L_l/(2*pi*params.R_l) * tan(params.Gama_l); % total fiber windings in revolutions (when relaxed)

% Right FREE
params.B_r = params.L_r/cos(params.Gama_r);   % fiber length
