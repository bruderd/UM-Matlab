...
params.Pmin = 3;
params.Pmax = 3.0001;        % maximum pressure tested
...
for i = 1:res
    ...
    [x(i+1,1), x(i+1,2), x(i+1,3), x(i+1,4), x(i+1,5), x(i+1,6)]  = solve_FBhoop(P_des(i+1), x0, params.load);
end
...
for j = 1:res
    ...
    [x1bolt(j+1,1), x1bolt(j+1,2), x1bolt(j+1,3), x1bolt(j+1,4), x1bolt(j+1,5), x1bolt(j+1,6)] = solve_FBhoop(P_des(j+1), x0, params.load);
end
...
for k = 1:res
    ...
    [x2bolt(k+1,1), x2bolt(k+1,2), x2bolt(k+1,3), x2bolt(k+1,4), x2bolt(k+1,5), x2bolt(k+1,6)] = solve_FBhoop(P_des(k+1), x0, params.load);
end
