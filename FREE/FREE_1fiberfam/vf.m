...
%% THIS WORKS: Attempt to match ICRA: F_elast = [c1 c2 c3]*[L^2 L 1]?; M_elast = [c4 c5 c6]*[theta^2 theta 1];
% f = [-dP + 0.5*(u - P);...  % The constant in front of (u-P) is arbitrary 
%      c2*dL + 2*c1*L*dL + pi*r^2*dP + 2*pi*P*r*dr - 2*pi*cot(gama)^2*r^2*dP - 4*pi*cot(gama)^2*P*r*dr + 4*pi*cot(gama)*P*r^2*dgama*(cot(gama)^2 + 1);...
...
% df_ddxdt = [-1,                                                                                                                               0,                                                                                                           0,                                                                            0;...
%             pi*r^2 - 2*pi*r^2*cot(gama)^2, 4*pi*P*r^2*cot(gama)*(cot(gama)^2 + 1), 2*pi*P*r - 4*pi*P*r*cot(gama)^2, c2 + 2*L*c1;...
%             2*pi*r^3*cot(gama), (L*c5*(tan(gama)^2 + 1))/r - 2*pi*P*r^3*(cot(gama)^2 + 1) + (2*L^2*c4*tan(gama)*(tan(gama)^2 + 1))/r^2, 6*pi*P*r^2*cot(gama) - (2*L^2*c4*tan(gama)^2)/r^3 - (L*c5*tan(gama))/r^2, (c5*tan(gama))/r + (2*L*c4*tan(gama)^2)/r^2;...
%             0, sin(gama), 0, cos(gama0)/L0];
