...
elseif choice == 2
    ...
    params.modulus = [1200, 700];
    [loadbolts,loadNm,loadP,loadphi] = importfile_loads('data/load40deg_v2.csv',2,48);
elseif choice == 3
...
params.modulus = [800, 100]; % Young's and shear modulus of elastomer: [E, G]
...
for i = 1:res
    ...
    [x(i+1,1), x(i+1,2), x(i+1,3), x(i+1,4), x(i+1,5), x(i+1,6)]  = solve_FBstress(P_des(i+1), params);
end
...
for j = 1:res
    ...
    [x1bolt(j+1,1), x1bolt(j+1,2), x1bolt(j+1,3), x1bolt(j+1,4), x1bolt(j+1,5), x1bolt(j+1,6)] = solve_FBstress(P_des(j+1), params);
end
...
for k = 1:res
    ...
    [x2bolt(k+1,1), x2bolt(k+1,2), x2bolt(k+1,3), x2bolt(k+1,4), x2bolt(k+1,5), x2bolt(k+1,6)] = solve_FBstress(P_des(k+1), params);
end
