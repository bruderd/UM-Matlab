function [con, coneq, grad_con, grad_coneq] = implicit_Dynamics(s, params)
    ...
    for p = 1:N+1
        ...
        con(1, p) = x(1,p) - Pmax;           %pressure limitation
        con(2, p) = x(2,p) - 1.5;          %fiber angle cannot exceed 90 degrees  
        ...
        con(1, (N+1)+p) = -x(1,p) + 0;
        con(2, (N+1)+p) = -x(2,p) + -1.5;            %fiber angle cannot exceed -90 degrees    
