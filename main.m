...
params.x0 = [2 4 -1]';    %state vector initial condition
params.u0 = [1]';        %input vector initial condition

params.T = 11;   %final time
...
s_star = fmincon(@(s)obj_nocost(s, params), s0, [], [],[],[], lb, ub, @(s)dynamics_noinput(s,params), options);
