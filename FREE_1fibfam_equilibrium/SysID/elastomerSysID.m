...
% 50 deg FREE
P_50 = [0, 2.5, 4, 5, 6, 7.5, 8.5, 10, 11, 12.5, 13, 14.5, 12.5, 13.5, 11, 12, 10, 11, 8.5, 10, 7.5, 9, 6.5, 8, 5.4, 6.5,...
        4, 5, 3.4, 0];
dL_50 = [0, 0, 0, 0.004, 0.008, 0.024, 0.048, 0.06, 0.084, 0.104, 0.148, 0.18, 0.148, 0.164, 0.14, 0.148, 0.116,...
...
choice = 4; % MAKE YOUR CHOICE HERE
...
% reg_Fs = MultiPolyRegress(dL_norm',Fs',1);
% % reg_Fs = MultiPolyRegress(P',Fs',1);    % this fit works much better!
% reg_Ms = MultiPolyRegress(dphi_norm',Ms',1);
% 
% Fs_poly = polyfit(dL_norm',Fs',1);
% % Fs_poly = polyfit(P',Fs',1);    % this fit works much better!
% Ms_poly = polyfit(dphi_norm',Ms',1);
...
% hold on
% plot(dL_norm,Fs,'*')
% plot(linspace(0,0.04,100), polyval(Fs_poly, linspace(0,0.04,100)))
% hold off
...
% set(gca,'fontsize', 32);
% axis([0, 1, -1.5, 0]);
% xlabel('$\frac{\Phi}{L}$ (rad/in)', 'fontsize', 32, 'Interpreter', 'LaTex');
% ylabel('$M_{elast}$ (lb-in)', 'fontsize', 32, 'Interpreter', 'LaTex');
% hold on
% plot(dphi_norm,Ms,'*')
% % plot(linspace(0,1,100), polyval(Ms_poly, linspace(0,1,100)))
% hold off
