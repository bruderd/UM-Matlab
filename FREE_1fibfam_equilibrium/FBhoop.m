...
function FB = FBhoop(x, u, x_prev, x_rest)
...
E = 2;  % Young's modulus of latex (find via sysid)
G = 7;  % Shear modululs of latex (find via sysid)

sig_x = E * (L - L0)/L0;
sig_r = E * 2*pi*(r - r0)/r0;
tau_rx = G * r*phi/L;
...
force_balance = P*pi*r^2 - 2*T*cos(gama) - 2*pi*r*sig_x;
torque_balance = 2*r*T*sin(gama) - 2*pi*r^2*tau_rx;
