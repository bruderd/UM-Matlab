...
%                 zeta = [ zeta ; u ];    % just so lifting function works COMMENTED OUT FOR BILINEAR SYSID
%                 obj.params.nzeta = obj.params.nzeta + obj.params.m;
            ...
            fullBasis = zeta;  % first nzeta observables should always be the measured state and delays COMMENTED OUT TO DO BILINEAR SHAPE SYSID
%             fullBasis = [];   % COMMENTED IN TO DO BILINEAR SHAPE SYSID
...
%             Cy = eye(obj.params.N);     % FOR JUST HAVING SHAPE AS THE LIFTED STATE
...
%             out.Cz = Cz;    % recovers state with delays (i.e.zeta)---doesn't work because zeta is not embeded in lifted state, only y is
        ...
        % get_NLmodel (Extracts the A,B,C matrices from Koopman matrix)
        function [ out , obj ] = get_NLmodel( obj , koopData )
            %get_NLmodel: Defines the A, B, and C matrices that describe the nonlinear
            %lifted system z_y+ = A z_y + B z_u , y = Cz.
            %   out - struct with fields A, B, C, sys, params, ...
            %   obj.model - property of struct which stores the model
            
            UT = koopData.K';    % transpose of koopman operator
            
            % Find all rows/columns of K that have inputs in them
            u_terms_index = contains( string( obj.basis.full ) , 'u' );
            obj.basis.zx = obj.basis.full( ~u_terms_index );
            obj.basis.zu = obj.basis.full( u_terms_index );
            obj.lift.zx = matlabFunction(obj.basis.zx, 'Vars', { obj.params.x });
