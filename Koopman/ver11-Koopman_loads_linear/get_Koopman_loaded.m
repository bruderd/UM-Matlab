function [ U , koopData ] = get_Koopman_loaded( snapshotPairs , params )
...
for i = 1:length(x)
    psix = stateLift( x(i,:)' , w(i,:)' )';
    psiy = stateLift( y(i,:)' , w(i,:)' )';
    Px(i,:) = [ psix , u(i,:) ];
    Py(i,:) = [ psiy , u(i,:) ];     % exclude u from Py (could also use same u as Px
end
...
%% Solve for inital Koopman Operator with subset of data points
