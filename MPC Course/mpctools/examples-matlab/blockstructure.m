...
lag = f + lamg'*g + lamx'*x;
var = struct('x', x, 'lamx', lamx, 'lamg', lamg, 'lag', lag);

Lagrangian = casadi.Function('Lagrangian', var, {'x', 'lamg', 'lamx'}, {'lag'});
Jacobian = Lagrangian.fullJacobian();
Hessian = Jacobian.fullJacobian();

sparsity = Hessian.sparsity_out(0);
H = zeros(Hessian.size_out(0));
H(sparsity.find()) = 1;
