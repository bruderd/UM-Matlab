...
%
% * `verbosity` : Integer Between 0 and 12 [Default 0]
% * `timelimit` : Positive Scalar (in seconds) [Default 60]
% * `maxiter` : Positive Integer [Default $\infty$]
% * `isQP` : Boolean [Default False]
%
...
methods (Access=public)
    function self = ControlSolver(varargin)
        ...
        if isempty(parser)
            ...
            parser.add('noinit', false(), {'scalar', 'bool'});
            ...
            parser.add('varprops', struct(), 'struct');
            parser.add('conprops', struct(), 'struct');
        end
        ...
        self.discretevar = args.discretevar;
        ...
        self.varprops = args.varprops;
        self.conprops = args.conprops;
        ...
        % Build nlp.
        self.map.x = mpctools.VarLayout(self.varsym, 'fromsyms', self.varprops);
        self.map.g = mpctools.VarLayout(self.consym, 'fromsyms', self.conprops);
    ...
    function copiedsolver = copy(self)
        ...
        args.discretevar = self.discretevar;
        ...
        args.varprops = self.varprops;
        args.conprops = self.conprops;
    ...
    function init(self, varargin)
        ...
        defopts = struct('solver', self.solver, 'isQP', self.isQP, ...
                         ...
                         'discretevar', self.map.x.vec(self.discretevar));
    ...
    function problem = getQP(self, qpsolver, point)
        ...
        % Create a function for calculating derivative information.
        ...
        problem.Q = problem.Q + triu(problem.Q, 1)';
            % TODO: remove this if CasADi changes the behavior of factory.
        
        problem.obj = -2*problem.Q*x0 + full(grad_f0);
        problem.objcon = -x0'*problem.Q*x0 - problem.obj'*x0 + full(f0);
        ...
        problem.vtype(logical(self.map.x.vec(self.discretevar))) = 'I'; % Integers.
