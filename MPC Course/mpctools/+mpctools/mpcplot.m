...
%
% * `marker` : String ['o']
% * `collocmarker` : String ['x']
%
%   > Plot marker to use for state points and state collocation points. Can be
%   set to empty string (`''`) to not use any marker.
%
% * `umarker' : String ['']
%
%   > Plot marker for control actions. If set to 'yes', uses the value given in
%   `marker`. Default is empty for no marker.
%
% * `fig` : Figure Handle
%
%   > Figure handle to use for ploting. Default is to make a new figure. This
...
if isempty(parser)
    ...
    parser.add('plot', true(), {'scalar', 'bool'});
    parser.add('marker', 'o', 'str');
    parser.add('collocmarker', 'x', 'str');
    parser.add('umarker', '', 'str');
    parser.add('fig', []);
...
if args.plot
    ...
    for i = 1:Nx
        ...
        if usecollocation
            ...
            plot(vals.t, vals.x(i,:), args.marker, 'color', args.color);
            if ~isempty(args.collocmarker)
                plot(vals.tc, vals.xc(i,:), args.collocmarker, ...
                     'color', args.color);
            end
        else
            plot(vals.t, vals.x(i,:), [args.linestyle, args.marker], ...
                 'color', args.color);
        end
        if ~isempty(vals.xsp)
    ...
    if ~isempty(args.legend)
        ...
        legend('off');
        plot(NaN(), NaN(), [args.linestyle, args.marker], 'color', args.color, ...
             'DisplayName', args.legend);
        legend('show', 'location', args.legendloc);
        if Ncols == 2 && Nx ~= Nu
            axis('off');
        end
    end
