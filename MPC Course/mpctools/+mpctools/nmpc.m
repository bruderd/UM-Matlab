...
% * `l`: Casadi Function
%
%   > Gives the stage cost for the system. To include (possibly time-varing)
...
%   parameters (a single vector for each time point). All other entries are
%   treated as time-invariant vectors or matrices.
%
% * `funcargs` : Struct
%
...
% * `xf` : Column Vector
%
%   > Gives the terminal value for `x`. If provided, a termminal equality
...
if isempty(parser)
    ...
    parser.add('finaly', true(), {'scalar', 'bool'});
...
[sym, varlist, parlist] = mpctools.getocpsymbols(args.casaditype, argsizes, ...
                                                 N, varlist, par, true(), args.finaly);
...
if ~isempty(args.udiscrete)
    ...
    kwargs.discretevar = struct('u', repmat(args.udiscrete, 1, N.t));
end
