...
methods (Static)
    function x = checkarg(name, x, checks)
        ...
        for i = 1:length(checks)
            ...
            case {'bool', 'logical'}
                if ~islogical(x)
                    error('Argument %s must be boolean!', name);
                end
            case 'casadifunc'
            ...
            case 'func'
                if ~isa(x, 'function_handle') && ~mpctools.iscasadifunc(x)
                    error('%s must be a function handle or casadi.Function!', name);
                end
            otherwise
                if length(check) > 2 && isequal(check([1,end]), '<>')
                    if ~isa(x, check(2:(end - 1)))
                        error('Argument %s must be a %s object!', name, check);
                    end
                elseif isfield(is, check)
                    if ~is.(check)(x)
                        error('Argument %s must be a %s!', name, check);
                    end
                else
