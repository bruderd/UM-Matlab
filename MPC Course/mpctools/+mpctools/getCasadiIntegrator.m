...
%   a vector "x" and matrices for all other parameters (with time along the
...
% * `scalar` : Logical [True]
%
%   > Depreciated. Passing `true` is equivalent to 'casaditype' of 'SX', and
%   `false` is equivalent to 'MX'. New code should use 'casaditype' directly.
%
% * `solver` : String ['cvodes']
%
%   > Which solver to use for integration. Typical options are 'cvodes', 'idas',
%   'rk', and 'collocation'. Consult the CasADi documentation for more
%   information about these options.
%
% * `casaditype` : String ['SX']
%
%   > String 'SX' or 'MX' to decide which type of CasADi symbolic variables to
%   use in the ODE expression. Consult the CasADi User Guide for more details.
persistent parser
if isempty(parser)
    ...
    parser.add('scalar', true(), 'bool');
    parser.add('solver', 'cvodes', 'str');
    parser.add('casaditype', [], 'str');
end
