function [sym, varlist, parlist] = getocpsymbols(casaditype, shapes, N, ...
                                                 varlist, parval, ...
                                                 finalx, finaly)
% [sym, varnames, parnames] = getocpsymbols(shapes, N, varlist, parval,
...
elseif ~iscell(varlist)
    error('varlist must be a cell array!');
end
if nargin() < 6
    finalx = true();
end
...
% Sort out variables. Note that we have to check consistency with the shapes
% that were inferred from the functions.
[varshapes, varrepeats, defaultshapes] = ...
    mpctools.generalvariableshapes_(varlist, N, finalx, finaly);
cellfun(@(v) checkconsistency(v, shapes, varshapes), varlist);
