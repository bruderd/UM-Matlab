...
% * `fix_truncated_x`
%
% > Boolean that decides whether to fix the $x$ variables that are outside of
% the current horizon. Usually this is unnecessary, but it can help with
...
methods (Access=protected)
    ...
    function [lb_, ub_] = getvarbounds(self)
        % Returns bounds for variables, fixing any excluded x variables.
        lb_ = self.lb;
        ub_ = self.ub;
        t = (self.horizon + 2):(self.maxhorizon + 1);
        if ~isempty(t)
            xguess = self.guess.x(:,t);
            lb_.x(:,t) = xguess;
            ub_.x(:,t) = xguess;
        end
    end%function
