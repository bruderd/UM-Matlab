...
methods (Access = public)
    function self = VarLayout(firstarg, howfrom, props)
        % layout = VarLayout(syms, ['fromsyms'])
        % layout = VarLayout(vals, 'fromvals', [properties])
        % layout = VarLayout(sizes,'fromsizes', [properties])
        %
        % Initializes the layout of variables.
        %
        % In the first signature, syms should be a struct with each entry either
        % a single Casadi variable or a cell array of Casadi variables. In this
        % case, the properties are all inferred.
        %
        % In the second signature, vals should be a struct with each entry a
        % numeric array whose size is the total size of the variable. All
        % properties take their default values, unless otherwise specified in
        % the third input.
        %
        % In the third signature, sizes should be a struct with each entry
        % giving the size of the corresponding variable. properties are handled
        % as in the second signature.
        %
        % If provided, properties is a struct whose entries give special
        % properties for each variable. Each entry should be a single string
        % or a cell array of strings from the following list:
        %
        %  - 'matrix'/'vector' : Specifies whether each variable is a matrix or
        %                        a vector (default 'vector')
        %
        %  - 'timevarying'/'constant' : Specifies if variable is time-varying
        %                               or constant (default 'timevarying')
        %
        %  - 'ignore' : Specifies that this variable should be ignored and not
        %               included in the layout.
        %
        % If conflicting properties are given, the last value is used. Note that
        % properties are completely ignored if the 'fromsyms' signature is
        % used.
        narginchk(1, 3);
        ...
        names_ = self.sortnames(fieldnames(firstarg));
        keep = true(size(names_));
        for i = 1:length(names_)
            thisprops = mpctools.structget(props, names_{i}, {});
            keep(i) = ~ismember({'ignore'}, thisprops);
        end
        self.names = names_(keep);
        self.Nnames = length(self.names);
        self.allinds = 1:self.Nnames;
        
        % Decide other properties.
        ...
        case 'fromsyms'
            for i = self.allinds
                ...
                if ndims(thissym) > 2 %#ok<ISMAT>
                    error('Symbolic variable "%s" has >2 dimensions!', name);
                end
                self.ismat(i) = size(thissym, 2) > 1;
            end
        case {'fromvals', 'fromsizes'}
            for i = self.allinds
                thisprop = mpctools.structget(props, self.names{i}, {});
                [self.ismat(i), self.isconst(i)] = self.parseprops(thisprop);
            end
        otherwise
            ...
            case 'fromsyms'
                ...
                if self.isconst(i)
                    ...
                    if isscalar(thissym)
                        ...
                        if cellsize(1) == 1
                            cellsize = cellsize(2:end); % Strip singletons.
                                % TODO: check that this covers edge cases.
                        end
                        %cellsize = cellsize(cellsize ~= 1);
                    end
                    self.sizes{i} = [varsize, cellsize];
                end
            case 'fromvals'
                self.sizes{i} = size(firstarg.(name)); % Just get size.
            case 'fromsizes'
                s = firstarg.(name); % Current size.
                if ~isvector(s) || ~isequal(s, round(s))
                    error('Each size entry must be a vector of integers!')
                elseif length(s) < 2
                    error('All sizes must have length >= 2!');
                end
            end
        end
...
methods (Access = private)
    function calcmap(self)
        ...
        % Finally, tack on variables that don't have a time component.
    ...
    function v = vec_numeric(self, s, vtype)
        ...
        % a single vector v using the precomputed indices.
...
methods (Static, Access = private)
    ...
    function [ismat_, isconst_] = parseprops(props, default)
        % Checks the list of properties and returns the corresponding booleans.
        narginchk(1, 2)
        if nargin() < 2
            default = false();
        end
        ...
        for i = 1:numel(props)
            ...
            case 'constant'
                isconst_ = true();
            otherwise
                error('Unknown property: %s', props{i});
            end
        end
    end%function
end%methods
