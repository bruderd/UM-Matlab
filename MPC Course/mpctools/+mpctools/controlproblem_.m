...
if isempty(parser)
    ...
    parser.add('discretevar', struct(), 'struct');
    parser.add('discretel', true(), {'scalar', 'bool'});
...
discretevar = mpctools.varstruct2numstruct(var, false(), varndims);
discretevar = updatebounds(discretevar, args.discretevar);

% Get function arguments and expand symbols for constraints.
funcargs = args.funcargs;
if usecollocation
    symc = mpctools.collocmerge_(sym, funcargs);
end
...
if isfield(args.funcs, 'f') && ~isempty(args.funcs.f)
    ...
    if usecollocation
        ...
        if args.singleshooting
            model = struct('con', {var.x(:,2:end)}, 'lb', -inf(N.x, N.t), ...
                           'ub', inf(N.x, N.t));
            var.x = var.x(1);
            lb.x = lb.x(:,1);
            ub.x = ub.x(:,1);
            guess.x = guess.x(:,1);
            discretevar.x = discretevar.x(:,1);
        else
...
allcon = struct('model', model, deltacon{:}, ...
                ...
                abscon{:}, args.con{:});
[con, conlb, conub, conprops] = mpctools.constraintmerge(allcon);
...
kwargs = struct('horizon', N.t, 'var', var, 'varlb', lb, 'varub', ub, ...
                ...
                'singleshooting', args.singleshooting, 'conprops', conprops);
