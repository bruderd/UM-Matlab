...
methods (Access=public)
    ...
    function [xhat_, dhat_] = filter(self, y, xhatm_, dhatm_, u_)
        ...
        e = y - (self.C*xhatm_ + self.Cd*dhatm_ + self.D*u_);
    ...
    function [xhatm_, dhatm_] = predict(self, u_, xhat_, dhat_)
        ...
        xhatm_ = self.A*xhat_ + self.B*u_ + self.Bd*dhat_;
    ...
    function [xtarg, utarg] = target(self, varargin)
        ...
        if isdefault.rsp
            ...
            args.rsp = self.H*args.ysp;
        end
        if isdefault.dhat
            args.dhat = self.dhat;
        end
        xtarg = self.Txd*args.dhat + self.Txr*args.rsp;
        utarg = self.Tud*args.dhat + self.Tur*args.rsp;
    end%function
end%methods
