...
%% ONLY 2 CASES. Fix: put in missing factor of 2 (2/13/2017)
% gama, betta have same sign
T_gama = (4*P*r^2*pi*(2*cot(gama) - cot(betta) + cos((pi*tan(betta))/tan(gama))*cot(betta)))/(sin(gama)*(cos((pi*tan(betta))/tan(gama)) - 2*nrat + cos(pi*(nrat - tan(gama)/tan(betta))) - cos((pi*tan(betta))/tan(gama))*cos(pi*(nrat - tan(gama)/tan(betta))) + 2*nrat*cos((pi*tan(betta))/tan(gama)) + 3));
T_betta = (4*P*r^2*pi*(2*cot(betta) - cot(gama) + cos(pi*(nrat - tan(gama)/tan(betta)))*cot(gama) - 2*nrat*cot(gama)))/(sin(betta)*(cos((pi*tan(betta))/tan(gama)) - 2*nrat + cos(pi*(nrat - tan(gama)/tan(betta))) - cos((pi*tan(betta))/tan(gama))*cos(pi*(nrat - tan(gama)/tan(betta))) + 2*nrat*cos((pi*tan(betta))/tan(gama)) + 3));
...
force_balance = diff( 0 == P*pi*r^2 - 2*(T_gama*cos(gama) + T_betta*cos(betta)) + F_elast ,  t);   
torque_balance = diff( 0 == 2*r*(T_gama*sin(gama) + T_betta*sin(betta)) + M_elast ,  t);              % put (r) in front of tensions to fix units (2/2/2017)           
...
dynamics_same = [force_balance; torque_balance; geometry_constraint1; geometry_constraint2; geometry_constraint3];

% gama, betta have different sign
T_gama = (4*P*r^2*pi*(cot(betta) + 2*cot(gama) - cos((pi*tan(betta))/tan(gama))*cot(betta)))/(sin(gama)*(cos((pi*tan(betta))/tan(gama)) - 2*nrat + cos(pi*(nrat + tan(gama)/tan(betta))) - cos((pi*tan(betta))/tan(gama))*cos(pi*(nrat + tan(gama)/tan(betta))) + 2*nrat*cos((pi*tan(betta))/tan(gama)) + 3));
T_betta = (4*P*r^2*pi*(2*cot(betta) + cot(gama) - cos(pi*(nrat + tan(gama)/tan(betta)))*cot(gama) + 2*nrat*cot(gama)))/(sin(betta)*(cos((pi*tan(betta))/tan(gama)) - 2*nrat + cos(pi*(nrat + tan(gama)/tan(betta))) - cos((pi*tan(betta))/tan(gama))*cos(pi*(nrat + tan(gama)/tan(betta))) + 2*nrat*cos((pi*tan(betta))/tan(gama)) + 3));
theta_gama0 = -tan(gama0)*L0/r0;     % (-) fixes sign convention (1/28/2017)
theta_betta0 = -tan(betta0)*L0/r0;   % (-) fixes sign convention (1/28/2017)
theta_gama = -tan(gama)*L/r;       % (-) fixes sign convention (1/28/2017)
...
dynamics_dif = [force_balance; torque_balance; geometry_constraint1; geometry_constraint2; geometry_constraint3];
